...
print(forward_tensor_concat)
print(concat_GR_f)
exit()
forward_concat = tf.concat([forward_tensor_concat, concat_GR_f, concat_POS_f], 2)
backward_concat =  tf.concat([backward_tensor_concat, concat_GR_r, concat_POS_r], 2)

nb_filter1 = 300 
nb_filter2 = 300

W2 = tf.get_variable("Filter_SDP", shape=[5, 200, nb_filter1],
...
W4 = tf.get_variable("W_soft", shape=[600, num_classes],
...
l2_loss = tf.multiply(0.000001, l2_loss)
...
with tf.Session() as sess:
    ...
    for epoch_idx in range(len(SGD_lrate1)):
        ...
        with open('rec_cnv4_predictions', 'w') as f:
            for item in preds:
                f.write("%s\n" % item)
        #with open('tf_rnn_cpoy6_copy_targets', 'w') as f:
        #    for item in target:
        #        f.write("%s\n" % item)        
        os.system('python torch_eval.py rec_cnv4_predictions test_target_modified')
    print(accuracies)    
