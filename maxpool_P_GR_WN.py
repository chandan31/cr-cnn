...
output_conc = tf.concat([max_pooled_forward, POS_max_pooled_f, GR_max_pooled_f, Wnet_max_pooled_f, max_pooled_backward, POS_max_pooled_r, GR_max_pooled_r, Wnet_max_pooled_r], 1)
...
with tf.Session() as sess:
    ...
    for epoch_idx in range(len(SGD_lrate4)):
        ...
        with open('maxpool_P_GR_WN_valid_preds', 'w') as f:
            for item in val_preds:
                f.write("%s\n" % item)
        
        os.system('python torch_eval_valid.py maxpool_P_GR_WN_valid_preds valid_targets %d'%epoch_idx)
        ...
        model_path = './save/maxpool_P_GR_WN_valid/model.ckpt' + str(epoch_idx)
        print(model_path)
        ...
        with open('maxpool_P_GR_WN_test_preds', 'w') as f:
            for item in preds:
                f.write("%s\n" % item)
        #with open('tf_rnn_cpoy6_copy_targets', 'w') as f:
        #    for item in target:
        #        f.write("%s\n" % item)        
        os.system('python torch_eval.py maxpool_P_GR_WN_test_preds test_target_modified %d'%epoch_idx)
    print(accuracies)    
