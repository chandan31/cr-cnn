...
input_conc = tf.concat([Embedding_dropout_left, input_GR_forward], 2)
input_conc_rev = tf.concat([Embedding_dropout_right, input_GR_backward], 2)


inputs_forward = [tf.squeeze(input_, [1]) for input_ in tf.split(input_conc, truncated_backprop_length, 1)]
inputs_backward = [tf.squeeze(input_, [1]) for input_ in tf.split(input_conc_rev, truncated_backprop_length, 1)]
...
output_cnv2 = tf.nn.conv1d(value=backward_tensor_concat, filters=W2, stride=1, padding='SAME')
...
output_conc = tf.nn.dropout(output_conc, 0.5)
...
L2_loss = tf.nn.l2_loss(l[0]) + tf.nn.l2_loss(l[1]) + tf.nn.l2_loss(l[2]) + tf.nn.l2_loss(l[4]) + tf.nn.l2_loss(l[8]) 
...
with tf.Session() as sess:
    ...
    for epoch_idx in range(len(SGD_lrate1)):
        ...
        with open('rec_cnv_3_1_predictions', 'w') as f:
            for item in preds:
                f.write("%s\n" % item)
        #with open('tf_rnn_cpoy6_copy_targets', 'w') as f:
        #    for item in target:
        #        f.write("%s\n" % item)        
        os.system('python torch_eval.py rec_cnv_3_1_predictions test_target_modified')
    print(accuracies)    
