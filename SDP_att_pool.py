...
W2 = tf.Variable(np.random.rand(4*state_size, num_classes),dtype=tf.float32)
...
output_att = tf.concat([forward_rep, backward_rep], 1)
...
output_pooled = tf.concat([max_pooled_forward, max_pooled_backward], 1)

output_conc = tf.concat([output_pooled, output_att], 1)
print(output_conc)
...
with tf.Session() as sess:
    ...
    for epoch_idx in range(len(SGD_lrate3)):
        ...
        with open('SDP_att_pool_predictions', 'w') as f:
            for item in preds:
                f.write("%s\n" % item)
        #with open('tf_rnn_cpoy6_copy_targets', 'w') as f:
        #    for item in target:
        #        f.write("%s\n" % item)        
        os.system('python torch_eval.py SDP_att_pool_predictions test_target_modified %d'%epoch_idx)
    print(accuracies)    
