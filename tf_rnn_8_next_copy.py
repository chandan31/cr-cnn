...
W2 = tf.Variable(np.random.rand(2*(state_size + pos_rec_1 + wn_rec_1), num_classes),dtype=tf.float32)
b2 = tf.Variable(np.zeros((1,num_classes)), dtype=tf.float32)
'''
W2 = tf.get_variable(
            name='W2',
            shape=[2*(state_size + pos_rec_1 + wn_rec_1), num_classes],
...
WNET_dropout_left = tf.nn.dropout(input_WNET_forward, keep_probability)
WNET_dropout_right = tf.nn.dropout(input_WNET_backward, keep_probability)
...
inputs_backward_WNET = [tf.squeeze(input_, [1]) for input_ in tf.split(input_POS_backward   , truncated_backprop_length, 1)]
...
# In what order this concat operation must occurr ?

output_conc = tf.concat([max_pooled_forward, max_pooled_backward, POS_max_pooled_f, POS_max_pooled_r, Wnet_max_pooled_f, Wnet_max_pooled_r], 1)
...
with tf.Session() as sess:
    ...
    for epoch_idx in range(50):
        ...
        with open('tf_rnn_8_next_level_predictions', 'w') as f:
            for item in preds:
                f.write("%s\n" % item)
        #with open('tf_rnn_cpoy6_copy_targets', 'w') as f:
        #    for item in target:
        #        f.write("%s\n" % item)        
        os.system('python torch_eval.py tf_rnn_8_next_level_predictions test_target_modified')
    print(accuracies)    
