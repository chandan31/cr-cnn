...
print("Load dataset")
f = gzip.open('pkl/sem-relations_1.pkl.gz', 'rb')
data = pkl.load(f)
f.close()
...
output = Dense(n_out, activation='softmax',kernel_regularizer=regularizers.l2(0.01))(output)
...
for epoch in range(nb_epoch):       
    ...
    pred_test = np.add(pred_test, 1)
    with open('cnn_predictions', 'w') as f:
            for item in pred_test:
                f.write("%s\n" % item)

    os.system('python torch_eval.py cnn_predictions test_target')
