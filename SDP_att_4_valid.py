...
W2 = tf.Variable(np.random.rand(2*(state_size + pos_rec_1 + gr_rec_1 + wn_rec_1), num_classes),dtype=tf.float32)
...
input_POS_forward = tf.nn.dropout(input_POS_forward, keep_probability)
input_POS_backward = tf.nn.dropout(input_POS_backward, keep_probability)

input_GR_forward = tf.nn.dropout(input_GR_forward, keep_probability)
input_GR_backward = tf.nn.dropout(input_GR_backward, keep_probability)


input_WNET_forward = tf.nn.dropout(input_WNET_forward, keep_probability)
input_WNET_backward = tf.nn.dropout(input_WNET_backward, keep_probability)
...
forward_concat = tf.concat([forward_tensor_concat, concat_POS_f, concat_GR_f, concat_Wnet_f], 2)
backward_concat =  tf.concat([backward_tensor_concat, concat_POS_r, concat_GR_r, concat_Wnet_r], 2)
...
H = forward_concat
M = tf.tanh(H)
att_w = tf.get_variable('A_W', shape=[state_size+pos_rec_1+gr_rec_1+wn_rec_1], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, trainable=True)
alpha = tf.nn.softmax(tf.tensordot(M, att_w, axes=(2,0)))

alpha1 = tf.stack([alpha for i in xrange(state_size + pos_rec_1 + gr_rec_1 + wn_rec_1)])
...
M1 = tf.tanh(H1)
att_w1 = tf.get_variable('A_W1', shape=[state_size+pos_rec_1+gr_rec_1+wn_rec_1], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, trainable=True)
alpha2 = tf.nn.softmax(tf.tensordot(M1, att_w1, axes=(2,0)))
alpha21 = tf.stack([alpha2 for i in xrange(state_size + pos_rec_1 + gr_rec_1 + wn_rec_1)])
...
#model_path = './save/SDP_att_1_valid/model.ckpt'
...
with tf.Session() as sess:
    ...
    for epoch_idx in range(len(SGD_lrate4)):
        ...
        valid_predictions, valid_accuracy, valid_loss  = sess.run(
                ...
                batchX_placeholder_WNET: Wnet_X_valid_f,
                batchX_placeholder_WNET_rev: Wnet_X_valid_r,
                l_rate: 0.001                    
                }
            )
        ...
        test_predictions, test_accuracy, test_loss  = sess.run(
            ...
            feed_dict={
                ...
                batchX_placeholder_WNET: Wnet_X_test_f,
                batchX_placeholder_WNET_rev: Wnet_X_test_r,
                keep_probability: 1.0,
                keep_probability2: 1.0,
                l_rate: .001        
            }
        ...
        model_path = './save/SDP_att_3_valid/model.ckpt' + str(epoch_idx)
        print(model_path)
...
# Added all four channels in the attention model. Dropout on all embeddings 