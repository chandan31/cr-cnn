...
state_size = 150
num_classes = 19
batch_size = 60  # Original 50 gave 84.35 F-1 without validation
...
r =  tf.multiply(M, tf.transpose(alpha1,[1, 2, 0])) 
...
r1 = tf.multiply(M1, tf.transpose(alpha21, [1, 2, 0] ))

#backward_rep = tf.tanh(r1) is it necessary ?

# One CNN over cocat tensor or 2 seperate CNNs ?
...
output_cnv1 = tf.nn.conv1d(value=r, filters=W2, stride=1, padding='SAME')
...
output_cnv2 = tf.nn.conv1d(value=r1, filters=W2, stride=1, padding='SAME')
...
#b = tf.Variable(tf.constant(0.1, shape=[nb_filter]), name="b")
b4 = tf.get_variable(name="Bias_Softmax", shape=[num_classes], initializer=tf.zeros_initializer(), trainable=True)
...
with tf.Session() as sess:
    ...
    for epoch_idx in range(len(SGD_lrate1)):
        ...
        epoch_loss = 0
        num_batches = int(13913/batch_size)
        ...
        with open('rec_att_cnv_predictions', 'w') as f:
            for item in preds:
                f.write("%s\n" % item)
        #with open('tf_rnn_cpoy6_copy_targets', 'w') as f:
        #    for item in target:
        #        f.write("%s\n" % item)        
        os.system('python torch_eval.py rec_att_cnv_predictions test_target_modified')
    print(accuracies)    
