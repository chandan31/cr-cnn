...
#c1 = tf.contrib.rnn.BasicLSTMCell(state_size, state_is_tuple=True)
#lstm_dropout = tf.contrib.rnn.DropoutWrapper(c1,input_keep_prob=keep_prob, output_keep_prob=keep_prob)


cell = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.BasicLSTMCell(state_size, state_is_tuple=True) for _ in range(2)])
cell1 = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.BasicLSTMCell(state_size, state_is_tuple=True) for _ in range(2)])
...
cell_GR_f = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.BasicLSTMCell(gr_rec_1, state_is_tuple=True) for _ in range(2)])
cell_GR_r = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.BasicLSTMCell(gr_rec_1, state_is_tuple=True) for _ in range(2)])
...
with tf.Session() as sess:
    ...
    for epoch_idx in range(len(SGD_lrate1)):
        ...
        with open('rec_cnv_multi', 'w') as f:
            for item in preds:
                f.write("%s\n" % item)
        #with open('tf_rnn_cpoy6_copy_targets', 'w') as f:
        #    for item in target:
        #        f.write("%s\n" % item)        
        os.system('python torch_eval.py rec_cnv_multi test_target_modified')
    print(accuracies)    
