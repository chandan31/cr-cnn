...
r = math.sqrt(6.0/(19.0+nb_filter))
w_classes = np.random.uniform(-r, r, size=(1000, 19))

print(r)
print(w_classes)


W_classes = tf.get_variable(name="class_embedings", shape=w_classes.shape, 
                        initializer=tf.constant_initializer(w_classes), trainable=True, dtype='float32')

class_scores = tf.matmul(pooled, W_classes)

correct_pred = tf.equal(tf.argmax(class_scores,1), tf.argmax(Y,1))
...
top2_val, top2_i = tf.nn.top_k(class_scores, 2, sorted=True)  
#true_score = top2_val[:,0]
#neg_score = top2_val[:,1]

true_score, _ = tf.nn.top_k(tf.multiply(class_scores, tf.cast(Y, dtype='float32')), 1)

neg_score, _ = tf.nn.top_k(tf.multiply(class_scores, tf.cast(1 - Y, dtype='float32')), 1) 
...
loss = tf.reduce_mean(tf.log(1 + tf.exp(tf.multiply(gamma, m_plus - true_score))) + tf.log(1 + tf.exp(tf.multiply(gamma, m_minus + neg_score))))
...
with tf.Session() as sess:
    ...
    for epoch_idx in range(nb_epoch):
        ...
        epoch_acc = 0.0
        epoch_loss = 0.0
        #sess.run(print('class scores', class_scores))
        #sess.run(class_scores)
        ...
        for batch_idx in range(num_batches):
            ...
            _class_scores, _total_loss, _train_step,  _accuracy = sess.run(
                    [class_scores, loss, train_step, accuracy],
                    feed_dict={
                        X: batchX, 
                        X_pos1: batch_pos1,
                        X_pos2: batch_pos2,
            ...
            #print('Class Scores', _class_scores)
