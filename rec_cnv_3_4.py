...
forward_concat = tf.concat([forward_tensor_concat, concat_GR_f, concat_Wnet_f], 2)
backward_concat =  tf.concat([backward_tensor_concat, concat_GR_r, concat_Wnet_r], 2)
...
with tf.Session() as sess:
    ...
    for epoch_idx in range(len(SGD_lrate1)):
        ...
        with open('rec_cnv_3_4_predictions', 'w') as f:
            for item in preds:
                f.write("%s\n" % item)
        #with open('tf_rnn_cpoy6_copy_targets', 'w') as f:
        #    for item in target:
        #        f.write("%s\n" % item)        
        os.system('python torch_eval.py rec_cnv_3_4_predictions test_target_modified')
    print(accuracies)    
